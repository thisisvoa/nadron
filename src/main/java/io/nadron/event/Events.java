...
public class Events {
    ...
    public static final byte NEW_USER_LOGIN = 0x26;

    public static final byte CHAT_MESSAGE = 0x28;
     ...
     *
     * @param source The payload of the event. This is the actual data that gets
     ...
     * @param source           The payload of the event. This is the actual data that gets
     *                         transmitted to remote machine.
     * @param deliveryGuaranty This decides which transport TCP or UDP to be used to send the
    ...
    public static NewUserLoginEvent newUserLoginEvent(Object source) {
        Event event = event(source, Events.NEW_USER_LOGIN);
        DefaultNewUserLoginEvent newUserLoginEvent = new DefaultNewUserLoginEvent(event);
        newUserLoginEvent.setDeliveryGuaranty(DeliveryGuaranty.DeliveryGuarantyOptions.RELIABLE);
        return newUserLoginEvent;
    }

    public static ChatMessageEvent chatMessageEvent(Object source) {
        Event event = event(source, Events.CHAT_MESSAGE);
        DefaultChatMessageEvent chatMessageEvent = new DefaultChatMessageEvent(event);
        chatMessageEvent.setDeliveryGuaranty(DeliveryGuaranty.DeliveryGuarantyOptions.RELIABLE);
